...
use sea_orm::{ActiveValue::NotSet, ColumnTrait, Condition, DbConn, EntityTrait, Iterable, QueryFilter, QuerySelect, TryIntoModel};

use self::dto::{CreateUserDto, UserQuery, UserResponse};
...
impl UserService {
	pub async fn find_one(conn: &DbConn, id: i32) -> Result<UserResponse, ApiError> {
		...
		match user {
			Some(v) => Ok(v),
			None => Err(ApiError::UserNotFound(id.to_string())),
		}
	}
